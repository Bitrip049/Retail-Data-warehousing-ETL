...
ctx= snowflake.connector.connect(
    ...
    role='accountadmin',
    warehouse='training',
    database='BITRIP_BHATBHATENI_DWH'
...
cs.execute("create or replace table D_BHATBHATENI_LOCN_T(id NUMBER,region_id NUMBER,store_desc VARCHAR(256),PRIMARY KEY (id),FOREIGN KEY (region_id) references D_BHATBHATENI_RGN_T(id) );")
...
cs.execute("create or replace table D_BHATBHATENI_PDT_T(id NUMBER,subcategory_id NUMBER,product_desc VARCHAR(256),PRIMARY KEY (id),FOREIGN KEY (subcategory_id) references D_BHATBHATENI_SUB_CTGRY_T(id));")
...
cs.execute("""
create or replace TABLE D_BHATBHATENI_LOCN_T (
	LOCN_ID NUMBER(38,0),
	...
	constraint RGN_FK foreign key (RGN_KY) references BHATBHATENI_DWH.BHATBHATENI_DWH.D_BHATBHATENI_RGN_T(RGN_KY)
);
""")
#
cs.execute("""
CREATE OR REPLACE TABLE D_BHATBHATENI_CTGRY_T
(
...
#
cs.execute("""
CREATE OR REPLACE TABLE D_BHATBHATENI_SUB_CTGRY_T
(
...
cs.execute("""
CREATE  OR REPLACE TABLE D_BHATBHATENI_PDT_T
(
...
#
cs.execute("""
CREATE or replace TABLE  D_BHATBHATENI_CUSTOMER_T
(
...
""")
cs.execute("""
CREATE  or replace  TABLE D_BHATBHATENI_TIME_YEAR_T
(
  ID NUMBER,
...
""")
cs.execute("""
CREATE or replace TABLE D_BHATBHATENI_TIME_DAY_T
(
  ID NUMBER NOT NULL,
  DAY_KY NUMBER,
  MONTH_KY NUMBER,
...
(
  ...
  CONSTRAINT LOC_FK FOREIGN KEY (LOCN_KY) REFERENCES D_BHATBHATENI_LOCN_T(LOCN_KY),
	...
	CONSTRAINT MONTH_FK FOREIGN KEY (MONTH_KY) REFERENCES D_BHATBHATENI_TIME_MONTH_T(MONTH_KY)
);''')
cs.close()
